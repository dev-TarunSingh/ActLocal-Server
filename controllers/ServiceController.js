...
export const NearbyServices = async (req, res) => {
  ...
  try {
    ...
    }).populate("postedBy", "firstName"); // Populating postedBy with firstName
    ...
    res.status(200).json({
      services: services.map((service) => ({
        ...
        postedBy: {
          _id: service.postedBy._id, // User's ObjectId
          firstName: service.postedBy.firstName, // User's first name
        },
      })),
    });
  } catch (error) {
...
export const RemoveServices = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error removing service: " + error.message });
  }
};
